...
async def create_class(request: ClassRequest, class_service: ClassService = Depends(get_class_service)):
    try:
        ...
        return ApiResponse(code=status.HTTP_422_UNPROCESSABLE_ENTITY, status="error", result=f"Create class failed: {str(e)}")
...
async def update_class(id: str = Path(...), request: ClassRequest = Body(), class_service: ClassService = Depends(get_class_service)):
    try:
        ...
        return ApiResponse(code=status.HTTP_404_NOT_FOUND, status="error", result=f"Class not found: {str(e)}")
    except Exception as e:
        return ApiResponse(code=status.HTTP_500_INTERNAL_SERVER_ERROR, status="error", result="Internal server error, please contact admin.")
...
async def delete_class(id: str = Query(...), class_service: ClassService = Depends(get_class_service)):
    try:
        ...
        return ApiResponse(code=status.HTTP_404_NOT_FOUND, status="error", result=f"Delete class failed: {str(e)}")
    except Exception as e:
        return ApiResponse(code=status.HTTP_500_INTERNAL_SERVER_ERROR, status="error", result="Internal server error, please contact admin.") 
